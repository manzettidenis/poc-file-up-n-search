...
export class FileSystemFileRepository implements IFileRepository {
  ...
  private async saveToDisk(): Promise<void> {
    ...
    try {
      await this.ensureDataDirectory()
      const filesArray = Array.from(this.files.values()).map(f => f.toJSON())
  ...
  async save(file: File): Promise<File> {
    ...
    try {
      ...
      // Cache file content
      FileContentCache.set(file.id, file.toJSON())
    ...
    try {
      ...
      if (cached) {
        const file = new File(
          ...
          cached.metadata,
          new Date(cached.uploadedAt),
          new Date(cached.lastModified),
        )
        return file
      }
      ...
      // Cache the result
      if (file) {
        FileContentCache.set(id, file.toJSON())
      }
  ...
  async update(file: File): Promise<File> {
    ...
    try {
      ...
      // Invalidate caches
      CacheManager.invalidateFileRelated(file.id)
